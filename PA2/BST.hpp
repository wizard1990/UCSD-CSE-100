...
protected:
    ...
    BSTNode<Data>* findFatherIfNotExist(const Data& item) const{
        ...
        while (tmpNode)
        {
            if (tmpNode->data < item) {     //search the right subtree
                if (tmpNode->right == 0)
                {
                    break;
                }
...
public:
  ...
  virtual bool insert(const Data& item) {
	  ...
	  if (!tmpNode)
	  {
		  root = new BSTNode<Data>(item);   //insert the root
		  isize = 1;
		  return true;
	  }
	  else
	  {
		  if (tmpNode->data == item)    //node exists
		  {
			  return false;
		  }
		  else
		  {
			  BSTNode<Data> *newNode = new BSTNode<Data>(item);     //insert new node
